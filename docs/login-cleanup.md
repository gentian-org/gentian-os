...
The theme goes first so it can be reviewed against stock Keycloak before any
authentication behaviour changes. Steps 2 and 3 are the switch-over and belong
together. Step 5 is deliberately last: while the grant still exists, reverting
step 3 restores the old behaviour without a redeploy of anything else.
...
Today the same query returns several one-shot sessions with `lastAccess == start`.

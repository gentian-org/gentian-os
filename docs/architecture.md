...
| **OpenTofu** (via Tofu Controller) | Infrastructure plane | Static kernel provisioning, secret seeding, external resources (DNS, cloud LBs, TLS CAs) | React to runtime events, manage tenant lifecycle |
...
| App deployment (Helm charts) | Wrong tool | Wrong tool | **Best fit** — drift detection, rollback, sync |
...
Credential rotation is passive: the orchestrator rotates credentials in OpenBao, and ESO automatically syncs the new values into Kubernetes Secrets. ArgoCD detects the changed Secret and rolls out the update. This is triggered via annotation (`kubectl annotate tenant acme-corp gentianos.io/rotate-credentials=all`).
...
sequenceDiagram
    ...
    AC->>AC: Deploy Helm chart (refs existingSecrets)
```
...
|-------|----------|------------|
| 100 | Namespaces, OpenBao, cert-manager, ESO | Layer 000 |
...
# Part III — Case Study: openDesk

## 10. openDesk Overview
...
### 10.1 openDesk Components Mapped to Gentian OS
...
### 10.3 openDesk Technology Stack
...
## 11. CRD Definitions for openDesk

### 11.1 AppProfile (with schema-based value mapping)
...
### 11.3 IntegrationBinding (auto-generated)
...
### 11.4 ExternalSecret (generated by orchestrator)
...
### 11.5 ArgoCD Application (generated by orchestrator)
...
## 12. OpenTofu — Kernel Seeding for openDesk
...
## 13. Orchestrator Reconciliation Logic

### 13.1 On Tenant Create/Update
...
## 14. Building the Orchestrator

### 14.1 Project Structure (Kubebuilder scaffold)

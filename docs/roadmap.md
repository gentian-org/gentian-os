...
* **Backlog Items**:
  - `[ ]` Commit the `provider-kubernetes` kind list as data, unioned from the sources above.
  - `[ ]` Generate its `ClusterRole` from that list and replace its `cluster-admin` binding. Resource names come from the API server, not from pluralised kinds.
  - `[ ]` Add a CI check failing when a render fixture carries a kind the list omits.
  - `[ ]` State the escalation-prevention ceiling for `provider-helm` in `provider-rbac.yaml`, so its grant reads as a limit of chart-shipped RBAC rather than as work not yet done.

...
### `hack/`
Licence-header templates used by `controller-gen`, `golangci-lint` (goheader),
and `scripts/normalize-go-headers.sh`.
...
| `infra/packages/` | A classic Helm repo (`index.yaml` + `.tgz`) served straight from raw.githubusercontent, regenerated by `scripts/publish-infra-charts.sh`. provider-helm `Release`s pull from here. |
...
- `lib/` — the sourced library (`load.sh` is the single entrypoint; `common.sh`,
  `argocd.sh`, `certs.sh`, `openbao.sh`, `catalogue.sh`, `portforward.sh`).
  Historic siblings `lib-runtime.sh`, `mail-lib.sh`, `llm-lib.sh`, and
  `verify-kernel-services.sh` sit one level up but are sourced by the same
  `load.sh`.
- One-shot helpers invoked by the installer: `install-argocd.sh`,
  `seed-openbao.sh`, `init-openbao-transit.sh`, `create-staging-ca-secret.sh`,
  `portal-login-bootstrap.sh`, `create-deployments-git-credentials.sh`.
- Generators run by `make`: `gen-clusterrole.py` (renders the chart ClusterRole
  from controller-gen RBAC output) and `gen-keycloak-theme-configmap.py`.
- `kubectl-gentian` — the CLI plugin (`gtnctl` symlink), installed with
  `make install-plugin`.

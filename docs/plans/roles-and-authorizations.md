...
| kernel | **Security officer** | `gentian:platform:security` | approving what escapes the default posture: privilege requests (MAC waivers, egress beyond baseline, elevated roles), the cluster exposure policy — allowed modes per trust tier, lifetimes, review interval — and catalogue entries at `trustTier: platform`; reviewing the decision and change logs and the cluster-wide exposure view | director (approval endpoints, `/v1/clusters/{c}/exposure`), read access to the three audit logs |

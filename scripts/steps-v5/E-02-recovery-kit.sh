...
# cluster when its login path is what broke. E-03 deletes the file the recovery
...
# summary would say a kit was still needed, and the recovery key was one reboot
# from gone the whole time.
#
# Until now v5 had no such step at all, so every v5 install ended with no kit
# and `--status` reporting one outstanding forever.
#
...
# The kit's own reads name a namespace only through these, and both default to
# v4's names, so exporting them is the whole of the layout work. Same pattern
# as B-08.
_kit_export_layout() {
    export GENTIAN_SYSTEM_NAMESPACE OPENBAO_NAMESPACE
...
apply() {
    ...
    # by now — B-08 seeded them — so a scoped run of this step alone works too.
    local out
    out="$(_kit_path)"

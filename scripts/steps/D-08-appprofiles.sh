...
#   Repository/apps claim that actually composes the catalogue-sync
...
apply() {
    ...
    # Repository/apps (phase B runs before phase D in every normal
    ...
    # so fail instead.
    kubectl get repository.gentianos.io apps -n crossplane-system >/dev/null 2>&1 || {
        error "Repository/apps not found. Run B-12-apps-repository first — it owns this claim."
        return 1
    }
}

destroy() {
    # Repository/apps belongs to B-12-apps-repository.sh, which tears it down

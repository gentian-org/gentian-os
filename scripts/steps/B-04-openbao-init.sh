...
# mutates: OpenBao storage; writes the local openbao-init file (E-04 removes it)
# pins: openbao
...
# no-op, discovered while making E-04 delete the local init file wholesale
# instead of stripping one field from it.
#
# Deliberately not replaced with something that deletes the local init file
# either. On a cluster that finished handover it is already gone — E-04

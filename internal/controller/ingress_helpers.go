...
more_set_headers "Content-Security-Policy: frame-ancestors 'self' %s";`, portalOrigin)
}
...
// ensurePortalEmbeddingAnnotations merges portal iframe CSP into ingress annotations.
// All operator-managed app ingresses get frame-ancestors for portal.<kernelDomain>
// because tenants authenticate via the shared kernel portal, not portal.<tenantDomain>.
func ensurePortalEmbeddingAnnotations(annotations map[string]string, kernelDomain string) {
	if kernelDomain == "" {
		return
	}

...
## Assert every manifest the installer and gtnctl SCAFFOLD validates against the
## CRDs this repo ships.
##
## A scaffold arrives already written, with comments explaining itself, so its
## fields read as decided rather than guessed. gtnctl wrote spec.mail.quotaPerUser
## and spec.mail.rateLimit into every tenant-defaults component; TenantMail has
## neither, CRDs reject an unknown field outright, and so every first tenant

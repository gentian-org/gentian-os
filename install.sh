...
# so uninstall.sh can use the same implementations. Sourced before any helper
# that depends on them. SCRIPT_DIR isn't set yet at this point in the file,
# so resolve our own location explicitly.
__GENTIAN_INSTALL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=scripts/lib-runtime.sh
source "${__GENTIAN_INSTALL_DIR}/scripts/lib-runtime.sh"

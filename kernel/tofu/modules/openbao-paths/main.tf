...
resource "random_password" "pg_postgres"               { length = local.pw_length; special = false; upper = false }
resource "random_password" "pg_keycloak"               { length = local.pw_length; special = false; upper = false }
resource "random_password" "pg_keycloak_extensions"    { length = local.pw_length; special = false; upper = false }
resource "random_password" "pg_selfservice"            { length = local.pw_length; special = false; upper = false }
resource "random_password" "pg_authsession"            { length = local.pw_length; special = false; upper = false }
resource "random_password" "pg_guardian"               { length = local.pw_length; special = false; upper = false }
resource "random_password" "pg_notifications"          { length = local.pw_length; special = false; upper = false }

resource "random_password" "mariadb_root"              { length = local.pw_length; special = false; upper = false }
resource "random_password" "mariadb_openxchange"       { length = local.pw_length; special = false; upper = false }

resource "random_password" "redis_password"            { length = local.pw_length; special = false; upper = false }

resource "random_password" "minio_root"                { length = local.pw_length; special = false; upper = false }
resource "random_password" "minio_nextcloud"           { length = local.pw_length; special = false; upper = false }
resource "random_password" "minio_openxchange"         { length = local.pw_length; special = false; upper = false }
resource "random_password" "minio_dovecot"             { length = local.pw_length; special = false; upper = false }
resource "random_password" "minio_migrations"          { length = local.pw_length; special = false; upper = false }
resource "random_password" "minio_notes"               { length = local.pw_length; special = false; upper = false }
resource "random_password" "minio_openproject"         { length = local.pw_length; special = false; upper = false }
resource "random_password" "minio_ums"                 { length = local.pw_length; special = false; upper = false }

resource "random_password" "nubus_master"              { length = local.pw_length; special = false; upper = false }
resource "random_password" "nubus_admin"               { length = local.pw_length; special = false; upper = false }
resource "random_password" "nubus_ldap_admin"          { length = local.pw_length; special = false; upper = false }
resource "random_password" "nubus_keycloak_admin"      { length = local.pw_length; special = false; upper = false }
resource "random_password" "nubus_ox_system_user"      { length = local.pw_length; special = false; upper = false }
resource "random_password" "nubus_smtp"                { length = local.pw_length; special = false; upper = false }
resource "random_password" "nubus_portal_shared"       { length = local.pw_length; special = false; upper = false }
resource "random_password" "nubus_minio_ums_key"       { length = local.pw_length; special = false; upper = false }
# LDAP search users
resource "random_password" "ldap_keycloak"             { length = local.pw_length; special = false; upper = false }
resource "random_password" "ldap_nextcloud"            { length = local.pw_length; special = false; upper = false }
resource "random_password" "ldap_dovecot"              { length = local.pw_length; special = false; upper = false }
resource "random_password" "ldap_element"              { length = local.pw_length; special = false; upper = false }
resource "random_password" "ldap_ox"                   { length = local.pw_length; special = false; upper = false }
resource "random_password" "ldap_postfix"              { length = local.pw_length; special = false; upper = false }
resource "random_password" "ldap_openproject"          { length = local.pw_length; special = false; upper = false }
resource "random_password" "ldap_xwiki"                { length = local.pw_length; special = false; upper = false }
# NATS — no digits allowed at position 0; prefix with 'n' below
resource "random_password" "nats_api"                  { length = local.pw_length; special = false; upper = false }
resource "random_password" "nats_dispatcher"           { length = local.pw_length; special = false; upper = false }
resource "random_password" "nats_prefill"              { length = local.pw_length; special = false; upper = false }
resource "random_password" "nats_udm_listener"         { length = local.pw_length; special = false; upper = false }
resource "random_password" "nats_udm_transformer"      { length = local.pw_length; special = false; upper = false }
# PG passwords stored in nubus secret (nubus reads them from nubus-credentials ESO secret)
resource "random_password" "nubus_pg_selfservice"      { length = local.pw_length; special = false; upper = false }
resource "random_password" "nubus_pg_authsession"      { length = local.pw_length; special = false; upper = false }
resource "random_password" "nubus_pg_keycloak"         { length = local.pw_length; special = false; upper = false }
resource "random_password" "nubus_pg_keycloak_ext"     { length = local.pw_length; special = false; upper = false }
resource "random_password" "nubus_pg_guardian"         { length = local.pw_length; special = false; upper = false }
resource "random_password" "nubus_pg_notifications"    { length = local.pw_length; special = false; upper = false }

resource "random_password" "keycloak_bootstrap_admin"  { length = local.pw_length; special = false; upper = false }
resource "random_password" "intercom_client_secret"    { length = local.pw_length; special = false; upper = false }

resource "random_password" "intercom_oidc_secret"      { length = local.pw_length; special = false; upper = false }
resource "random_password" "intercom_matrix_as"        { length = local.pw_length; special = false; upper = false }
resource "random_password" "intercom_redis"            { length = local.pw_length; special = false; upper = false }
resource "random_password" "intercom_portal_shared"    { length = local.pw_length; special = false; upper = false }
resource "random_password" "intercom_session"          { length = local.pw_length; special = false; upper = false }

resource "random_password" "nextcloud_admin"           { length = local.pw_length; special = false; upper = false }
resource "random_password" "nextcloud_metrics"         { length = local.pw_length; special = false; upper = false }
resource "random_password" "nextcloud_status"          { length = local.pw_length; special = false; upper = false }

# OX App Suite
resource "random_password" "ox_admin"                  { length = local.pw_length; special = false; upper = false }
resource "random_password" "ox_hz_group"               { length = local.pw_length; special = false; upper = false }
resource "random_password" "ox_basic_auth"             { length = local.pw_length; special = false; upper = false }
resource "random_password" "ox_jolokia"                { length = local.pw_length; special = false; upper = false }
resource "random_password" "ox_cookie_hash_salt"       { length = local.pw_length; special = false; upper = false }
resource "random_password" "ox_share_crypt_key"        { length = local.pw_length; special = false; upper = false }
resource "random_password" "ox_sessiond_key"           { length = local.pw_length; special = false; upper = false }
resource "random_password" "ox_oidc_client_secret"     { length = local.pw_length; special = false; upper = false }
resource "random_password" "ox_connector_api_password" { length = local.pw_length; special = false; upper = false }

resource "random_password" "dovecot_doveadm"           { length = local.pw_length; special = false; upper = false }
resource "random_password" "dovecot_oidc_client_secret" { length = local.pw_length; special = false; upper = false }
